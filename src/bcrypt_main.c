...
//bcryptxp
typedef BOOLEAN (APIENTRY *RtlGenRandomFn)(PVOID RandomBuffer, ULONG RandomBufferLength);
...
NTSTATUS WINAPI BCryptGenRandom( BCRYPT_ALG_HANDLE handle, UCHAR *buffer, ULONG count, ULONG flags )
{
    ...
    if (alg || (flags & BCRYPT_USE_SYSTEM_PREFERRED_RNG))
    {
        ...
        static RtlGenRandomFn pRtlGenRandom;

        if (hModule == NULL) {
            hModule = GetModuleHandleA("Advapi32.dll"); // existing module.
            if (hModule == NULL) {
                LoadLibraryA("Advapi32.dll"); // attempt load; handle leak, one-time.
            }
            if (hModule != NULL) {
                pRtlGenRandom = (RtlGenRandomFn) // resolve symbol
                    GetProcAddress(hModule, "SystemFunction036");
            }
            if (pRtlGenRandom == NULL) {
                FIXME("unable to resolve SystemFunction036\n");
            }
        }
        if (pRtlGenRandom && pRtlGenRandom(buffer, count)) return STATUS_SUCCESS;
#else
